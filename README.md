### Production Deployments using Kubernetes and Helm

#### Abstract

Shifting application development, testing, and hosting from bare metal to containers leads to a better software-development lifecycle because systems become immutable, disposable, recreatable, and portable.  Likewise, moving from monolithic systems to microservices promotes strong encapsulation, single responsibility, load distribution and scaling, quick to-production turnaround time, isolated failures, and access control.  Plus, microservices remove the binding to a single technology and architecture.  For example, an authorization layer written in Typescript with a MySQL backend, a CMS written in Go and storing documents in MongoDB, and a computer vision system using Python and Tensorflow all co-existing as part of a larger system.  But despite the advantages of containers and microservices, distributed systems such as the one described above come bearing a new complexity: deployment.  Services need to be deployed in the correct order, auto-scale under load, and restart in the event of a crash.  Developers should be able to quickly rollback a service to a known-stable state in the event of a defect.  Services may need to be geographically redundant.  There should be an audit trail so that systems engineers can review each deployment.  And, maybe most importantly, deployments should be stored as code inside the VCS and should integrate seamlessly with CI/CD pipelines.  These challenges led to the advent of container orchestration platforms, the most notable of which is Kubernetes (k8s).  All major cloud-hosting providers, including Google Cloud, AWS, Digital Ocean, and Microsoft Azure, now support k8s, and its rapid adoption has made it the de facto standard for the dev-ops industry.

#### Goal

Create a load-balanced API that depends on a database, and then deploy that API to a local k8s cluster.
...
If you want, you can get a bash shell running in the MySQL container and take a look around.  (Your pod name will be different than mine.)
